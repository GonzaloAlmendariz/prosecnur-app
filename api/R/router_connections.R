...
.connections_provider_label <- function(provider) {
  ...
  switch(provider, surveymonkey = "SurveyMonkey", kobo = "KoboToolbox")
}
...
.connections_load_profile_manifest <- function(provider) {
  ...
  # sm_token. Lo exponemos como perfil "Principal" sin migrar secretos.
...
.connections_save_profile_manifest <- function(provider, manifest) {
  ...
  clean <- list(
    ...
    profiles = lapply(profiles, function(p) list(
      id = .connections_profile_clean_id(p$id %||% ""),
...
.connections_active_profile <- function(provider) {
  manifest <- .connections_load_profile_manifest(provider)
  default_id <- .connections_profile_clean_id(manifest$default_profile_id %||% "")
...
.connections_token_state <- function(provider, sid = NULL) {
  provider <- .connections_normalize_provider(provider)
  secret_name <- .connections_secret_name(provider)
  ...
  active_profile <- if (.connections_profiles_supported(provider)) .connections_active_profile(provider) else NULL
...
.connections_token_status <- function(provider, sid = NULL) {
  .connections_token_state(provider, sid)$status
}

.connections_token_require <- function(provider, sid = NULL) {
  provider <- .connections_normalize_provider(provider)
  state <- .connections_token_state(provider, sid)
...
.connections_check_kobo <- function(sid = NULL, base_url = NULL) {
  token <- .connections_token_require("kobo", sid)
  base <- .kobo_api_trim_base_url(base_url %||% kobo_api_default_base_url())
...
.connections_check <- function(provider, sid = NULL, base_url = NULL) {
  provider <- .connections_normalize_provider(provider)
  if (identical(provider, "surveymonkey")) return(.connections_check_surveymonkey(sid))
  .connections_check_kobo(sid, base_url = base_url)
}
...
mount_connections <- function(pr) {
  pr |>
    ...
    plumber::pr_post("/api/connections/<provider>/check", wrap_endpoint(function(req, res, provider, ...) {
      parsed <- .connections_parse_body(req)
      .connections_check(provider, session_header(req), base_url = parsed$base_url %||% parsed$baseUrl %||% NULL)
    })) |>
