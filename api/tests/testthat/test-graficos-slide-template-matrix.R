...
test_that("matriz ACNUR v2 acredita 28 tipos, 18 layouts y wire path-free", {
  ...
  expect_identical(length(matrix$slides), 28L)
  expect_identical(length(unique(vapply(matrix$slides, `[[`, character(1), "layout"))), 25L)
...
test_that("aliases compatibles resuelven 28 de 28 dentro del mismo master", {
  preferred <- vapply(
    .SLIDES_META,
  ...
  expect_identical(length(selected), 28L)
...
test_that("registry serializa render_key declarativo para los 28 tipos", {
  registry <- .graficos_registry_payload()
  slides <- registry$slides
  expect_identical(length(slides), 28L)
...
test_that("GET slide-layout-matrix monta y sirve el contrato v2 explícito", {
  ...
  expect_identical(length(payload$slides), 28L)
})
...
test_that("HTTP real absorbe query params y conserva req$argsQuery como autoridad", {
  ...
  expect_identical(length(matrix$json$slides), 28L)
...
.l7_sentinel_plan <- function() {
  ...
  list(
    ...
    p_slide_3_graficos_2mas1(
      blank, blank, blank, "L7-S19-graficos_3_2mas1"
    ),
    p_slide_3_graficos_1mas2(
      blank, blank, blank, "L7-S20-graficos_3_1mas2"
    ),
    p_slide_3_graficos_fila(
      blank, blank, blank, "L7-S21-graficos_3_fila"
    ),
    p_slide_3_graficos_1arriba(
      blank, blank, blank, "L7-S22-graficos_3_1arriba"
    ),
    p_slide_2_graficos_vertical(
      blank, blank, "L7-S23-graficos_2_vertical"
    ),
    p_slide_2_graficos_asimetrico(
      blank, blank, "L7-S24-graficos_2_asimetrico"
    ),
    p_slide_3_graficos_poblacion(
      blank, blank, blank, "L7-S25-poblacion-tres", icon
    ),
    p_slide_4_graficos_poblacion(
      blank, blank, blank, blank, "L7-S26-poblacion-cuatro", icon
    ),
    p_slide_5_graficos_poblacion(
      blank, blank, blank, blank, blank, "L7-S27-poblacion-cinco", icono = icon
    ),
    p_slide_6_graficos_poblacion(
      blank, blank, blank, blank, blank, blank,
      "L7-S28-poblacion-seis", icono = icon
    )
  )
}

test_that("renderer ACNUR consume el mismo objeto v2 en un deck de 28 sentinelas", {
  skip_if_not_installed("officer")
  ...
  expect_identical(length(doc$slide$names()), 28L)
  layout_meta <- doc$slideLayouts$get_metadata()
  actual_layouts <- vapply(seq_len(28L), function(i) {
  ...
  sentinels <- sprintf("L7-S%02d-", seq_len(28L))
