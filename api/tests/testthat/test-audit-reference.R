test_that("audit reference builds a portable .pulso and reopens in fresh sessions", {
  ...
  expect_equal(sum(grepl("^files/", zip_index$filename)), 2L)
  ...
  expect_true(all(vapply(saved_state$files, function(meta) {
    identical(meta$kind, "xlsform") || identical(meta$kind, "data")
  }, logical(1))))
