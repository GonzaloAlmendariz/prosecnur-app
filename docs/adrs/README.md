...
| [0035 — Editor](0081-editor-xlsform-coleccion-multi-formulario.md) | Aceptado | 2026-07-14 | El Editor XLSForm mantiene una coleccion multi-formulario; comparte numero historico con el ADR de Calculo de muestra |
...
| [0070](0070-el-namespace-lo-escribe-roxygen.md) | Aceptada | 2026-08-10 | `api/NAMESPACE` vuelve a ser generado: el pipe de magrittr se declara con `@importFrom` en `prosecnurapp-package.R` en vez de a mano, entran los 181 exports y los dos métodos S3 que el archivo nunca reflejó, los nombres con punto inicial dejan de exportarse, el bloque manual desaparece por redundante, y `Roxygen: list(roclets = c("namespace"))` evita generar `api/man/`. Un test de contrato compara `@export` contra el NAMESPACE en ambos sentidos |
...
| [0077](0077-avisa-fuerte-lo-que-sigue-produciendo-dano.md) | Aceptada | 2026-08-15 | Entre las señales de calidad de campo de Monitoreo, avisa fuerte la que **sigue produciendo daño** mientras nadie actúa; la que ya terminó de producirlo, informa. Solo `formulario_desactualizado` es bloqueante —una encuestadora que sigue con la versión vieja pierde una encuesta más por hora, y esas encuestas no se editan después—, mientras que nombres mal escritos, encuestas superpuestas o respuestas vacías son hechos ya ocurridos sobre un conjunto cerrado de casos. La app nunca frena el campo sola, toda alerta dice a quién llamar y qué preguntarle, y las de calidad viajan fuera de la lista de avance para que «te faltan 12 encuestas» y «Mary sigue con el formulario viejo» no se lean igual |
| [0078](0078-marcar-una-variable-abre-una-decision-no-la-cierra.md) | Aceptado | 2026-08-15 | Una codificación está completa cuando no le quedan variables marcadas **sin decidir**, no cuando todas tienen categorías. Marcar declara una intención y abre una decisión que se cierra de cuatro formas, y «no categorizar, con motivo» es una de ellas: hoy decidir no categorizar por `n` insuficiente no tiene dónde vivir y se confunde con un olvido. En ACNUR V3, 9 marcadas, 3 con categorías y 5 pendientes que se entregaron sin recodificar sin que nada lo dijera |

| [0079](0079-el-cumplimiento-se-mide-contra-la-cuota-del-diseno.md) | Aceptado | 2026-08-18 | El cumplimiento del estudio se mide contra la cuota del diseño; la meta operativa por aula sigue existiendo para dirigir el campo, y las dos nunca se publican bajo el mismo rótulo. Monitoreo medía el avance contra la suma de metas por aula (`expected_valid`), que está estructuralmente por encima de la cuota porque el dimensionamiento compra más aforo del que la cuota pide: el porcentaje subestimaba el cumplimiento y un estrato podía no llegar nunca a 100 % con su cuota cubierta. Medido en HSVG2026: una meta agregada publicada en 84.110 antes de acotarla a los slots (6.901). Entra el bloque `avance_cuota` con denominador `design_targets`; el numerador es todo lo recogido válido de la facultad —reservas activadas incluidas, que suman al numerador y nunca al denominador— con huérfanas, duplicados y fuera de universo declarados aparte. Los targets por sexo dejan de repartirse por la composición del frame de aulas, que sobre-representa a quien está en más aulas (1,55 aulas/alumno), y pasan a la sub-distribución que el diseño certificó |
| [0080](0080-el-workspace-se-resella-entero-no-campo-a-campo.md) | Propuesto | 2026-08-18 | Al aceptar una corrida como vigente, el backend resella el workspace entero en vez de sincronizar campo a campo. `workspace.aulas_config` y la copia de la config que viaja en el snapshot de la corrida se actualizan por caminos distintos, y cada camino que olvida un campo produce una divergencia que el gate de acreditación castiga después, a veces sin remedio desde la UI. Cinco mordidas medidas de la misma familia: `n_aulas` 202 contra 203 —que dejó mudo al relato de la selección y no se reparaba re-comparando, porque el campo viejo vivía en el workspace—, `simulation_runs` 500 contra 0, un preset mixto que perdía las claves sueltas, una invalidación que borraba las decisiones y dejaba la promoción, y el sentinela de alumnos/CH que es un resello parcial más. Hermano operativo del 0077: aquí se decide no producir la divergencia, no sólo avisarla |

| [0081](0081-editor-xlsform-coleccion-multi-formulario.md) | Aceptado | 2026-07-14 | El editor XLSForm aloja una colección de formularios por proyecto y materializa el activo como espejo del contrato legacy. El estado vivía en `s$xlsform_state`, un objeto opaco con un solo formulario, y cinco consumidores externos leen `s$xlsform_state$workbook` —handoff de Carga, handoff de Monitoreo, facts del Home, estado de módulos y warm-start—: migrarlos era superficie amplia sobre handoffs ya frágiles. La colección se construye alrededor del contrato legacy en vez de reemplazarlo, así que la homepage tipo biblioteca (crear, abrir, renombrar, eliminar y conmutar) llega sin tocar a los cinco. Numerado 0081 al indexarlo: nació el mismo día que el 0035 de calc-muestra y compartió su número durante un mes, y las referencias en código de «ADR 0035» son todas del otro |
