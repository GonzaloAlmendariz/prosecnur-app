...
| Acreditacion hydrated visual QA | Partial | Critical probes passed, but all-tab pass is 20/22 because `Telefono/Resumen` and `Telefono/Dia` remained on `Preparando vista`. |
...
| Acreditacion `Telefono > Dia` | Hydrated but data/visual failure; warning fixed | Initial `tmp/visual-qa/acreditacion-parity-telefono-8792-20260628/15-telefono-dia.png` emitted 65 duplicate-key warnings. After the key fix, `tmp/visual-qa/acreditacion-parity-telefono-dia-keyfix-8792-20260628/15-telefono-dia.png` has `console_errors=0` and `wait_ms=15114`, but original chart total `67` still diverges from modular daily totals `676/670`. |
...
| ACRDCONTA strict full pass after model repair | 21/22 hydrated, one loader failure remains | `tmp/visual-qa/acrconta-parity-full-after-model-8792-20260628/report.json` uses `/Users/gonzaloalmendariz/Documents/Pulso/pruebas-monitoreo/ACRDCONTA.pulso`; Fuentes 3/3, Modelo 5/5, Consultas 5/5, Telefono 3/4, Avance 5/5. `Telefono/Dia` fails because modular keeps loader/empty readiness text, not because it only lacks visual parity. |
