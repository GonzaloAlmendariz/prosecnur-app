...
| Comparador P1↔P2 de alcance estadístico + carga operativa (I20) | no existe; el selector oculta el otro escenario | **hecho (2026-08-07, veto levantado por revisión de contratos)**: emisor `api/R/calc_muestra_comparacion_escenarios.R` (schema `calc_muestra_comparacion_escenarios_v1`, owner `engine_r`), estampado en `alumnos_por_ch_calcular_estudio`, repair del carrier en `normalize_componente`; el TS valida y rechaza, nunca origina cifras; evidencia: `test-calc-muestra-comparacion-escenarios.R` verde (135 asserts, smoke HTTP + round-trip `.pulso`) y el normalizador de producción acredita ready/stale/legacy | ambos diseños comparables sin memorizar ni recalcular en React |
...
| Hallazgos abiertos del loop | 0 | **7** (carril A de motor en I21: A1 elegibles, A2 razones, A3/A7 cifras divergentes, A6 vacío de Alumnos/CH; I21b bloqueo del hilo; instrumento de QA anonimizado; D10 final I22 — el veto contractual I20 se levantó el 2026-08-07 con el emisor R) | = 0 |
