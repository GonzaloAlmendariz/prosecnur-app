...
| 20 | «P1 frente a P2» (comparador de costo de precisión) | libertad gobernada F0/F4 | **activa** — capacidad completa; auditoría es su primer paso, no un lote aparte |
| 21+ | Libertad creativa gobernada: lo que la evidencia pida (simulador «qué pasa si», comparador de criterios, export de radiografía), colgado de las cinco dimensiones y gateado por F0 | — | abierto |
...
| Comparador P1↔P2 con costo de precisión (I20) | no existe; el selector oculta el otro escenario | pendiente | ambos diseños comparables sin memorizar ni recalcular en React |
...
- **Baseline medido:** I18b deja sus gates verdes. I19 acredita sus dos direcciones en 1440×1000 y 1024×600: contradicción C5 anterior; CH tiene scroll anidado manual (panel + tabla) y Distribución no declara geometría en markup. Reportes `/private/tmp/prosecnur-i19-ch-baseline.iPK5wI/report.json` y `/private/tmp/prosecnur-i19-dist-geometry.53AeQ5/report.json`; procesos propios cerrados.
...
### Scope lock I20 — «P1 frente a P2» (comparador de costo de precisión)
...
- **Primera divergencia medida:** la superficie ofrece P1/P2 como toggle, pero acredita **0 comparaciones simultáneas**; para explicar el costo de precisión hay que memorizar muestra, CH y banda de un escenario antes de abrir el otro.
- **Capacidad de salida:** una lectura lado a lado muestra qué compra P2 frente a P1 en cuota planificada, cursos-horario requeridos y alcance de precisión formal, y permite saltar al detalle de cualquiera de los dos.
- **Carril A — contrato R:** un owner nuevo compara únicamente dos bundles `ready`, del mismo `source_frame_hash` y `population_hash`; publica valores P1/P2 y deltas firmados de muestra/CH, nunca resta hashes distintos ni mezcla fórmula con meta.
- **Semántica:** P1 conserva precisión formal global y diagnóstico por facultad; P2 conserva precisión formal por facultad. El comparador narra el intercambio, no declara ganador ni promete precisión formal por sexo.
- **Carril B — superficie:** secuencia pregunta → diferencia principal → costo operativo → alcance metodológico; ambas columnas permanecen visibles en desktop y se apilan con orden explícito en 1024×600, sin doble scroll.
- **Frescura:** si falta un escenario, cambia el frame o cualquiera queda `legacy|stale|invalid|incompatible`, la comparación falla cerrada y dirige a recalcular; no cae al motor TS ni al único escenario disponible.
- **Owners previstos:** helper/contrato R y test focal nuevos; normalizador/componente/test React nuevos; seams mínimos en el constructor de distribución y `CalculoDistribucionTab`.
...
- **Riesgo principal:** presentar una diferencia aritmética correcta sobre diseños no comparables o convertir “más muestra” en una recomendación metodológica implícita.
...
- **Stopping rule único:** P1 y P2 se comparan simultáneamente con valores y deltas R-owned, semántica formal correcta, fail-closed, acceso al detalle, dos viewports limpios, owners sin crecimiento extraíble, ledger/registro actualizados y commit atómico.
