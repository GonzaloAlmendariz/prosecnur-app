...
| P30 | **La N por barra no baja al gráfico** (pedido de Gonzalo) | graficadores de barras | ☐ **el motor promete algo que no hace**. La lámina metodológica que se acaba de recrear dice «en los casos en que una pregunta presenta un número de respuestas menor al total de la base, esta cifra se muestra directamente en el gráfico correspondiente» — y **el motor tiene cero** de esas anotaciones contra **11 del aprobado** (láminas 12, 13, 18, 19). Patrón dominante medido (7 de las 11, lámina 18): **8 pt, cursiva**, `left = 28.9 cm` fijo —justo a la derecha del final de la barra—, `w = 1.05`, `h = 0.26`, **una por fila**, con el `top` siguiendo a su barra. El otro patrón (lám 12): 10 pt, recta, caja de 4.78 cm. ◐ **helper construido** (`.n_barra_capa()`, 21 tests) con la anotación medida del aprobado y el criterio de «sólo la fila que no cuadra», tolerancia de un caso y sólo por debajo. **NO enganchado, con medición**: probado tomando como base la mayor N de la lámina —el graficador no conoce el pie— da **146 anotaciones contra 11**. Confunde **público pequeño** con **salto de cuestionario**: en una lámina con 52 docentes, 172 estudiantes y 15 administrativos anota los tres. Lo que el aprobado anota es la fila menor que **la base de su propio público**, y esa base existe en el motor de plan pero **no viaja al graficador**. **CRITERIO CORREGIDO al medir la lámina 18**: su base es «52 docentes, 172 estudiantes, 178 egresados y 15 administrativos» y las siete N anotadas son 47, 160, **172**, **15**, 30, 143 y **15** — tres **coinciden con la base de su público**. Luego la unidad **no es la fila, es la PREGUNTA**: se anotan todas sus filas en cuanto una salta, porque decir «N = 143» sólo en egresados deja sin saber cuántos son los demás. `.n_barra_procede_por_pregunta()` lo implementa. **La base por público NO hay que traerla de fuera: se deduce** — es el mayor número de respuestas que da ese público entre sus preguntas, porque una pregunta sin salto la responde entero. Comprobado contra la lámina 18: las cuatro bases declaradas (52, 172, 178, 15) salen **exactas**. **Pero el graficador no puede hacerlo**: se le llama **una vez por pregunta** —trazado, `.group_id` constante y un público por fila—, así que el máximo por público es su propia N y da **cero anotaciones**. Falta engancharlo en el **renderer de multilista**, que sí recorre todos los bloques (ya lo hace para el paso común de P14). Exige **dos pasadas** y **subir la línea base** de `reporte_plan_ppt.R` (9674 vs 9663) en `agentic/manifest.json` — que **otra sesión está editando ahora**. Decisión de gobierno |
