...
| P16 | **Dos láminas del mismo tipo no salen iguales** | firma de **B4** | ◐ **la vara medía de más**: su firma no distinguía familia y el grupo de «6 barras» juntaba cinco multi-apiladas con un **radar** — cada familia declara su propio alto de fila (0.54 in apiladas, 0.58 multi, 0.64 agrupadas) y su grosor difiere por diseño. Con la paleta en la firma: **motor 8, aprobado 5**. El defecto que queda es real: `rampa:4` dispersa **0.846 cm** en el motor contra 0.307 del aprobado. **Causa trazada** sobre las 238 llamadas del mazo: `alto_por_categoria` llega con **0.55, 0.651 y 0.74** según la lámina y la fracción es casi siempre 0.78, así que el grosor físico sale en 0.429 / 0.508 / 0.522 / 0.577 / 0.618 in. Quien reparte ese alto es el **llamador**, no el graficador. **Intento descartado**: `.grosor_anclado_al_nominal()` corregía el caso de `canvas_h_panel_in` impuesto, pero ese parámetro es `NULL` en todo el mazo y el ancla no se activaba nunca; el helper queda con sus tests |
...
| P22 | **La leyenda se parte encima del enunciado** (hallazgo del render) | `size_leyenda` del preset | ◐ **48 de 48 leyendas no cabían en su caja**, contra 8 de 53 del aprobado, por ir a **15.99 pt** contra 12.0. Exceso mediano **1.53× → 1.15×**; el aprobado está en 0.93×. **No cierra**: lo que queda es el **ancho de la caja**, no el tamaño |
