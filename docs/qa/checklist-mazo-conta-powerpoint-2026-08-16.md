...
| P29 | **Tres defectos hermanos del radar** (vistos al reparar P28) | radar + tabla nativa | ◐ **dos de tres cerrados**. **(b) leyenda en minúscula ☑**: se capitaliza sólo la copia que va al graficador — en el origen NO, porque de esos niveles cuelgan las claves de `tabla_encabezados`, que casan con `match()`, y un `docentes = "…"` del analista se quedaría sin columna en silencio. **(c) encabezados partidos ☑**: la columna de tema se llevaba el 47 % (6.36 de 13.53 cm) y las de datos quedaban en 2.39; con 0.40 suben a 2.70 y los partidos bajan de **2 a 1**. **Descartado por el camino**: repartir en proporción a la longitud del encabezado — ensanchar «Estudiantes» a 2.62 encoge «Docentes» a 2.20 y pasan a partirse las **tres**. **(a) sólo se ve una de las tres series ◐**: el **aprobado SÍ las distingue** —líneas finas y marcadores, comprobado renderizando su PPTX—. El grosor **es** la causa: con los mismos datos a 1.2 se funden y a 0.6 se distinguen. Pero `.radar_mb_linea_legible()` **se descartó**: trazado, el tope se aplica (1.1 → 0.6 en las seis llamadas) y **el render no cambia ni con 0.35**. El `size_linea` de esa llamada no pinta esas láminas: **hay otro camino de dibujo** y encontrarlo es el siguiente paso |
