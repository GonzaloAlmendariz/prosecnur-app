...
| P16 | **Dos láminas del mismo tipo no salen iguales** | alto de fila del llamador | ◐ **causa acotada, dos remedios descartados**. La vara medía de más y se corrigió (firma con paleta: motor 9→8, aprobado 6→5). El defecto que queda es real: `rampa:7` y `rampa:3` son de **una sola familia** y aun así dispersan. El salto es siempre **1.20×** (0.980 vs 1.173; 1.694 vs 2.068). **No es el grosor**: el preset usa `grosor_modo = "manual"`, la fracción es **0.7 fija** y `.auto_bar_width_apiladas()` ni se llama. Lo que varía es el **alto de fila**, y los números cuadran exactos — `0.55×0.7 = 0.978`, `0.651×0.7 = 1.19`, `0.74×0.7 = 1.36` cm. El preset declara 0.54 / 0.651 / 1.024, así que **0.55 y 0.74 los produce otra capa**: ahí está lo que falta encontrar |
