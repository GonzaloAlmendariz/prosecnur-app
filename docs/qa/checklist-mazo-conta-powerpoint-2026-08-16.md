...
| P46 | **22 enunciados cortados a media frase, en 16 láminas de 66** (el aprobado tiene CERO) | `.barras_acotar_titulo_grupo()` en `graficador_helpers_titulos_grupo.R` | ☐ **ABIERTO — el hallazgo más grande de la sesión, y salió mirando la 41.** Contados los `<a:t>` que terminan en «…»: motor **22 en 16 láminas** (25, 26, 29, 30, 31, 38, 39, 41, 43, 49, 57, 59, 60, 65, 68, 69), aprobado **0**. Una de cada cuatro láminas entrega una pregunta cortada. El motor **ya lo sabe y lo avisa**: 23 `[PULSO-AVISO] Enunciado recortado…` en la corrida. **La causa no es el wrap** —`.barras_wrap_titulo_grupo()` ya mide con `textGrob`, no estima—: es el **ancho del canal**. Medido en el XML de la 41, «La Unidad facilita los medios necesarios…» vive en **1.492 in a 14 pt**; el aprobado pone sus enunciados largos en cajas de hasta **6.675 in a 13 pt** (su lámina 35, ocho líneas sin recortar). **Más de cuatro veces el ancho y un punto menos de cuerpo.** El consejo del aviso —ensanchar «Columna de grupo»— es correcto pero es global a todas las multi-apiladas y se lo lleva el analista; lo que falta es que el motor ensanche cuando no cabe, igual que P43 hizo con la etiqueta de eje, y `.chars_que_caben()` ya está para decidirlo midiendo. **MEDIDO EL MISMO ENUNCIADO EN LOS DOS MAZOS.** «La Unidad facilita los medios necesarios para que…» sale en la **lámina 39 del aprobado** en un canal de **2.678 in a 13 pt, seis líneas, entero**; en la **41 del motor**, en **1.492 in a 14 pt, recortado**. Y el aprobado **no tiene tope de líneas por fila**: su lámina 29 llega a **nueve y hasta doce líneas a 12 pt** en canales de 2.4–3.2 in. Su regla no es «tantas líneas por fila» sino «el canal que el texto necesite, y si hace falta un punto menos de cuerpo». **◐ MEDIO CERRADO: el parámetro ya viaja, y no basta.** Trazado: la causa **no** era `w_group / w_sum` (la 41 llega con `w_sum = **1.02**` y fracción **0.2157**; el «0.22/1.85 = 0.119» era coincidencia aritmética) ni el 1.492 in del XML era el canal (es la caja del **texto**, que `draw_text` ajusta a lo dibujado). Lo real: **`ancho` llegaba valiendo 10** —el default de la firma de `graficar_barras_apiladas()`— mientras la lámina se dibuja a **12.511 in**; **20 llamadas** de la pasada de PPT en ese caso. El canal para envolver salía `0.2157 × 10 − 0.06 = 2.097 in` contra los **2.699** reales: el enunciado se envolvía un **22 %** más estrecho que su sitio. **Causa raíz**: en `multilista` cada subbloque es un `ppt_element` propio que se renderiza **sin pasar por `.render_element()`**, y el bucle ya heredaba `excluir_opciones`, `row_step_forzado`, `bases_publico` y `legend_key_aspect_yx` — el cajón simplemente **no estaba en esa lista cerrada**. **Reparado** con `.multilista_heredar_cajon()` (archivo propio; línea base del congelado **9716 → 9724**). **Verificado en el XML**: el enunciado de la 41 pasa de **1.492 a 1.794 in**, un 20 %. **Y NO BASTA, con su cifra: los truncados bajan sólo de 22 a 21.** La restricción que manda no es el envoltorio sino el **cupo** —`floor(n_filas × lpf × alto_rel)`—, y el aprobado **no tiene tope de líneas por fila** (nueve y doce líneas a 12 pt en su lámina 29). Eso sigue abierto. Se conserva porque la entrada era falsa y ahora es verdadera y **la vara no se mueve**: 21 → 21, B3 se redistribuye sin cambiar de cuenta y **el peor caso mejora** (lámina 41 de **0.43 a 0.34**; sale la 30, entra la 66 con 0.05; suma 1.42 → 1.41). Tests **15**, rojo comprobado (2 fallos). **La primera versión de la prueba de consumo NO rojeaba**: buscaba `.multilista_heredar_cajon(` a secas y el comentario de dos líneas más arriba nombra la función. Ahora busca la asignación completa. Commits `2af12d9b`+`8fa60752` |
