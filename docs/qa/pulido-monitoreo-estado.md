...
|---|---|---|---|---|---|
| **corrección de método** | territorial y acreditación | tablas de revisión | Los recortes en celda se venían dejando abiertos con el argumento de que «envolver redistribuye las columnas». **Comprobado en las tres tablas de territorial y es falso en las tres.** `register-table` no es una tabla sino una rejilla con columnas resueltas; `mon-territorial-review-table` **sí** es `<table>` pero declara `table-layout: fixed`, y con eso los anchos los fijan la primera fila y las anchuras explícitas, no el contenido —26 columnas medidas antes y después, idénticas—; Encuestadores ya no tenía recortes. El criterio que faltaba: **envolver solo puede mover columnas si la tabla es `table-layout: auto`**. Se mira la propiedad antes de decidir. Queda por medir así Cruces efectivos de acreditación (45) | medido el 2026-07-30 en `acnur_acg` | territorial cerrado; falta acreditación |
| ~~pendiente de comprobar~~ **NO APLICA** | acreditación | Teléfono › Responsables | Se sospechaba que «Equipo asignado» tenía el mismo defecto que en telefónico y **no lo tiene**. Cada página monta su propia copia del workbench (`AcreditacionMonitoreoPage.tsx:6372` frente a `TelefonicoMonitoreoPage.tsx:7291`) y han divergido: acreditación no monta `mon-phone-summary-secondary`, así que el bloque no comparte fila con otra columna —es el contenido entero de su pestaña, dentro de un `pulso-panel` que ya le da marco—. Se llegó a escribir la regla gemela y se retiró el mismo día: `matches()` sobre el elemento real da false. Duplicarla habría dejado CSS muerto y cajas concéntricas | comprobado en la app el 2026-07-30 | cerrado |
| duplicación estructural | telefónico y acreditación | workbench de teléfono | La función `AcreditacionPhoneOperationsWorkbench` **existe dos veces**, con el mismo nombre, en los dos page-files, y han divergido en su layout. Es la razón de fondo de que sus estilos vivan en hojas separadas y de que un arreglo en uno no llegue al otro | `TelefonicoMonitoreoPage.tsx:7291`, `AcreditacionMonitoreoPage.tsx:6372` | abierto |
| estructura | territorial | Modelo › Manzanas | **La fila del UMP seleccionado desaparece de la lista**: el ítem con `is-expanded` renderiza a 2 px de alto —`clientHeight` 0, solo bordes— mientras sus hermanos miden 64. La causa es `min-height: 0` sobre el expandido (`territorialProfile.css:416`) combinado con `overflow: hidden`: en un contenedor con overflow recortado, `min-height: auto` resuelve a 0, así que la pista de la rejilla colapsa. **No se arregla y conviene explicar por qué**: el contenido que oculta —«UMP 1 · ATE · Mz 0390 · Zona 00100 · Rango 1-8 · P824 · Revilla Ciriaco»— es exactamente el que ya muestra la Ficha UMP del panel derecho, así que no se pierde dato. Probado `min-height: max-content`: el ítem crece a 201 px pero **se superpone a UMP 2 y 3**, porque los hermanos no se desplazan. Decidir si la expansión inline sigue existiendo o si la fila debe conservar sus 64 px de trigger es estructura del componente | `territorialProfile.css:411-418`; medido el 2026-07-30 en `acnur_acg` | abierto |
| ~~contenido cortado~~ falso positivo | territorial | Modelo › Manzanas | Las otras 149 filas del acordeón dan `scrollHeight > clientHeight` porque **un acordeón colapsado oculta su contenido por diseño**. Es la undécima familia de falso positivo: los widgets de divulgación (`details`, acordeones) no son contenido perdido | 2026-07-30 | cerrado |
| recorte | territorial | Modelo › Cobertura | Con la leyenda ya visible se ve que **9 de sus etiquetas se recortan** —«Titul…», «Reem…», «SAN J…», «C…», «L…»— en 530 px. Son nombres de distrito y categorías del mapa; caben si la leyenda envuelve, pero eso cambia su alto y con él el reparto de la rejilla | medido el 2026-07-30 en `acnur_acg` | abierto |
...
| ~~deuda compartida~~ **SALDADA** | los cuatro modos | Avance › Salidas | `mon-outputs-*` tenía su escala duplicada por perfil —telefónico y territorial— porque nadie había mirado el archivo compartido. Ahí estaba la causa: `salidas/outputsWorkbench.css` declaraba `border-radius: var(--pulso-radius-md, 12px)`, **un token inexistente con fallback**, así que caía a 12 en los cuatro modos. Corregido en el compartido y **retirados los dos duplicados**; una sola declaración gobierna ahora los cuatro. Verificado en territorial y telefónico a la vez: 14 y 10 en ambos | `salidas/outputsWorkbench.css:1639`; 2026-07-30 | cerrado |
...
| ~~recorte en tabla~~ **RESUELTO** `d838d3e6` | territorial | Consultas internas | Los diez recortes —nueve «P597 · Vargas Carlos Angel» en la tabla de revisión y el chip de métricas— se arreglaron al comprobar que la tabla es `table-layout: fixed` y envolver no mueve sus 26 columnas | medido el 2026-07-30 | cerrado |
| ~~recorte en tabla~~ **YA NO OCURRE** | territorial | Fuente › Encuestadores | Remedida el 2026-07-30 con el detector corregido: **cero recortes** en la tabla. Los ocho de la primera pasada eran de menos de 4 px, dentro del umbral de subpíxel que ahora se excluye | medido el 2026-07-30 en `acnur_acg` | cerrado |
