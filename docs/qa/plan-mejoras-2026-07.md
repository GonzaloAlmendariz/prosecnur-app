...
| 4.2 | **Re-unificar el fork acred↔telefónico** | Core compartido parametrizado por familia para los pares con diff mínimo medido (`*ActorGoals` 8/55, `*PhoneDailyTrend` 22/247, `*PhoneAlerts` ~52%, `*SourcesModel`), y de ahí hacia arriba. Detiene el costo "cada fix ×2" antes de que diverja más. Requiere decidir primero qué diferencias son semánticas reales. | L | 8→1 (dominio + guardian-contratos congelan contrato; frontend-react ejecuta en rebanadas) |
