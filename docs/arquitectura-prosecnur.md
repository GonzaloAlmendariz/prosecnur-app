...
| Seguridad del shell | El renderer no hereda privilegios del proceso principal ni recibe secretos persistidos | IPC validado por ventana/frame/origen, navegación por allowlist, CSP, broker HF loopback, `safeStorage` real | Un secreto guardado nunca cruza a React; CDP y DevTools solo en desarrollo o auditoría con manifest y `userData` aislado |
