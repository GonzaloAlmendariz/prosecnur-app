...
// Poll del job de sincronización de fuentes para el perfil territorial
// (consola de fuentes: Hojas de Ruta vía sheets/sync async). Copia por perfil
// deliberada, con el mismo vocabulario que acreditacion/sync/pollSourceSync.ts
// y telefonico/sync/pollSourceSync.ts: cada perfil diverge por separado.

export type TerritorialSourceSyncProgressUpdate = {
  percent: number | null;
  phase: string;
  message: string;
};
...
export async function waitForSourceSyncJob(
  jobId: string,
  onProgress?: (progress: TerritorialSourceSyncProgressUpdate) => void,
  isCancelled?: () => boolean,
) {
  for (let attempt = 0; attempt < 90; attempt += 1) {
    await new Promise((resolve) => window.setTimeout(resolve, attempt === 0 ? 350 : 1000));
    // Corte de cancelación: sin él, el poll seguiría golpeando /api/jobs
    // hasta 90 s después de que el usuario saliera del módulo.
...
export function useWaitForSourceSyncJob() {
  ...
  return useCallback(
    (jobId: string, onProgress?: (progress: TerritorialSourceSyncProgressUpdate) => void) =>
