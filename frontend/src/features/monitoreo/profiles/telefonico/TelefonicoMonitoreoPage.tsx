...
function isTelefonicoVisibleAdvanceTab(tab: AcreditacionLocalTabKey): tab is AcreditacionAdvanceTab {
...
function inlineAdvanceDateTickLabel(value: string) {
  if (isAcreditacionNoDateLabel(value)) return "S/D";
  const parsed = parseAcreditacionDailyDate(value);
  if (!parsed) return shortAdvanceDateLabel(value);
  const month = parsed.toLocaleDateString("es-PE", { month: "short" }).replace(".", "").toLowerCase();
  return `${month} ${String(parsed.getDate()).padStart(2, "0")}`;
}
  ...
  const chartLayout = {
    ...
    annotations: [
      ...
      ...dailyLabelRows.map((point) => ({
        ...
        font: { color: "#168a55", size: isEffectiveGeneralChart ? 11 : 10, family: "Inter, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" },
      })),
  ...
  const prefetchBackgroundScopes = useCallback((view: WorkbenchView) => {
    ...
    scopes.forEach((scope, index) => {
      ...
      window.setTimeout(() => {
        ...
        if (stateByScopeRef.current.has(scope) || inFlightScopeRef.current.has(scope)) return;
        const request = apiMonitoreoState({
          includeReports: true,
          reportScope: scope,
          warmupCache: true,
        }).then((next) => {
          if (cacheEpoch !== scopeCacheEpochRef.current) return null;
          ...
          if (inFlightScopeRef.current.get(scope) === request) inFlightScopeRef.current.delete(scope);
        });
        inFlightScopeRef.current.set(scope, request);
        void request;
      }, route.family === "telefonico" ? 420 + index * 420 : 240 + index * 180);
    });
  }, [route.family]);
