...
type SheetKey = "survey" | "choices" | "settings" | "diagnostico";
...
type BuilderNodeKind = "section" | "repeat" | "question" | "note" | "calculate";
...
type DependencyKind = "relevant" | "constraint" | "calculation" | "choice_filter";

type XlsformDependency = {
  fromRowIndex: number;
  fromName: string;
  toName: string;
  kind: DependencyKind;
...
const SURVEY_COLUMNS = ["type", "name", "label", "hint", "required", "relevant", "constraint", "calculation", "choice_filter", "appearance"];
const CHOICES_COLUMNS = ["list_name", "name", "label"];
const SETTINGS_COLUMNS = ["form_title", "form_id", "version", "default_language"];
...
function makeSheet(name: string, columns: string[]): XlsformEditorSheet {
  return { name, columns: [...columns], rows: [] };
}

function createBlankWorkbook(): XlsformEditorWorkbook {
  return {
...
function cloneSheet(sheet: XlsformEditorSheet): XlsformEditorSheet {
  return {
...
function cloneWorkbook(book: XlsformEditorWorkbook): XlsformEditorWorkbook {
  return {
...
function getSheet(workbook: XlsformEditorWorkbook, key: SheetKey): XlsformEditorSheet | null {
  if (key === "diagnostico") return workbook.diagnostico ?? null;
  return workbook[key];
}

function ensureColumn(sheet: XlsformEditorSheet, columnName: string): number {
  let idx = sheet.columns.indexOf(columnName);
  if (idx >= 0) return idx;
...
function rowToRecord(sheet: XlsformEditorSheet, rowIndex: number): Record<string, string> {
...
function setCell(sheet: XlsformEditorSheet, rowIndex: number, columnName: string, value: string) {
...
function insertRecord(sheet: XlsformEditorSheet, index: number, record: Record<string, string>) {
...
function deleteRow(sheet: XlsformEditorSheet, rowIndex: number) {
  sheet.rows.splice(rowIndex, 1);
}

function parseType(rawType: string): TypeInfo {
...
function buildType(base: string, listName = "") {
...
function buildSimpleCondition(variableName: string, operator: string, value: string) {
...
function parseSimpleCondition(expression: string) {
...
function asRequired(raw: string) {
  const value = (raw ?? "").trim().toLowerCase();
  return value === "yes" || value === "true" || value === "1";
}

function cleanFilename(name: string | null | undefined, fallback = "instrumento_editado.xlsx") {
  const raw = (name ?? "").trim();
...
function slug(input: string, fallback = "campo") {
  const safe = (input ?? "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function typeLabel(baseType: string) {
  switch (baseType) {
    case "text":
      return "Texto corto";
    case "integer":
...
function sheetTitle(key: SheetKey) {
  switch (key) {
    case "survey":
      return "Survey";
    case "choices":
...
function sheetDescription(key: SheetKey) {
  switch (key) {
    case "survey":
...
function formatSource(kind: string | null) {
  switch (kind) {
    case "surveymonkey":
...
function makeColumnName(sheet: XlsformEditorSheet, prefix = "columna") {
...
function extractChoiceItems(choicesSheet: XlsformEditorSheet, listName: string): ChoiceItem[] {
...
function buildCatalogs(choicesSheet: XlsformEditorSheet): CatalogSummary[] {
  const listCol = choicesSheet.columns.indexOf("list_name");
  const nameCol = choicesSheet.columns.indexOf("name");
  const labelCol = choicesSheet.columns.indexOf("label");
  if (listCol < 0) return [];
...
function collectDependencies(node: BuilderNode): XlsformDependency[] {
  ...
  return sources.filter((item) => !!item.expression).flatMap((source) =>
...
function buildXlsformIndex(workbook: XlsformEditorWorkbook): XlsformIndex {
  ...
  const missingReferences = dependencies.filter((dependency) => !variablesByName.has(dependency.toName));
  ...
  return {
    ...
    missingReferences,
    stats: {
      nQuestions: structure.outline.filter((node) => node.kind === "question" || node.kind === "note" || node.kind === "calculate").length,
      nSections: Array.from(structure.sections.values()).filter((section) => section.kind !== "root").length,
...
function parseBuilderStructure(survey: XlsformEditorSheet): BuilderStructure {
  ...
  survey.rows.forEach((_, rowIndex) => {
    ...
    const label = (record.label ?? "").trim() || (record.name ?? "").trim() || `Elemento ${rowIndex + 1}`;
    ...
    const kind: BuilderNodeKind =
      typeInfo.base === "note" ? "note" :
      typeInfo.base === "calculate" ? "calculate" :
      "question";
  ...
  for (const id of stack.slice(1)) {
    ...
    spans.set(meta.rowIndex, { start: meta.rowIndex, end: Math.max(survey.rows.length - 1, meta.rowIndex) });
  }
...
function resolveInsertionIndex(structure: BuilderStructure | null, selection: BuilderSelection | null, survey: XlsformEditorSheet) {
  ...
  if (node.kind === "section" || node.kind === "repeat") {
    const section = Array.from(structure.sections.values()).find((entry) => entry.rowIndex === selection.rowIndex);
...
function previewKindLabel(node: BuilderNode | null) {
...
function getSiblingRows(structure: BuilderStructure | null, rowIndex: number | null) {
  if (!structure || rowIndex == null) return { prevRow: null as number | null, nextRow: null as number | null };
...
function buildDiagnostics(workbook: XlsformEditorWorkbook | null, index: XlsformIndex | null): BuilderDiagnostic[] {
  ...
  structure.outline.forEach((node) => {
    if (!(node.label ?? "").trim()) {
      diagnostics.push({
        ...
        detail: "Añade una etiqueta legible para que el formulario no quede opaco al momento de usarlo.",
      });
    }
    if (!(node.name ?? "").trim()) {
      diagnostics.push({
        ...
        detail: "El nombre interno ayuda a exportar, validar y reutilizar la información sin ambigüedad.",
      });
    } else if ((nameCounts.get(node.name) ?? 0) > 1 && firstRowByName.get(node.name) === node.rowIndex) {
      diagnostics.push({
    ...
    if (node.kind === "question" && (node.typeInfo.base === "select_one" || node.typeInfo.base === "select_multiple")) {
      if (!node.typeInfo.listName) {
        diagnostics.push({
          ...
          detail: "Asigna un catálogo de opciones para que esta pregunta de selección quede completa.",
        });
    ...
    if (node.kind === "calculate" && !node.calculation) {
      diagnostics.push({
        ...
        detail: "La variable está marcada como cálculo, pero todavía no tiene una fórmula definida.",
      });
    }
    if (node.kind === "repeat") {
      diagnostics.push({
        ...
        detail: "Por ahora el constructor no expone repeats de forma guiada; si necesitas ajustarlo, usa el modo avanzado.",
      });
    }
  });
  ...
  structure.unmatchedEndRows.forEach((rowIndex) => {
    diagnostics.push({
      ...
      detail: "Se detectó un end_group/end_repeat suelto. Conviene revisarlo antes de exportar.",
    });
  });
  ...
  catalogs.forEach((catalog) => {
    ...
    if (catalog.items.length < 2) {
      diagnostics.push({
        ...
        detail: "Si lo vas a usar en una pregunta de selección, normalmente conviene definir al menos dos opciones.",
      });
    }
    catalog.items.forEach((item) => {
      if (!(item.name ?? "").trim()) {
