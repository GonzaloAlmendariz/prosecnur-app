  ...
  survey.rows.forEach((_, rowIndex) => {
    ...
    if (typeInfo.base === "begin_group" || typeInfo.base === "begin_repeat") {
      ...
      const node: BuilderNode = {
        ...
        constraint_message: (record.constraint_message ?? "").trim(),
        ...
        mediaImage: (record["media::image"] ?? "").trim(),
        mediaAudio: (record["media::audio"] ?? "").trim(),
        mediaVideo: (record["media::video"] ?? "").trim(),
